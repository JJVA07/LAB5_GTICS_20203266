...
public class UserController {
    ...
    public String juego(Authentication auth, Model model) {
    ...
    @GetMapping("/user/ultimas-partidas")
    public String ultimasPartidas(Authentication auth, Model model) {
        MyUserDetails user = (MyUserDetails) auth.getPrincipal();
        model.addAttribute("nombre", user.getUsuario().getNombre());
        model.addAttribute("rol", user.getAuthorities().iterator().next().getAuthority().replace("ROLE_", ""));
        return "user/ultimas_partidas";
    }
}
